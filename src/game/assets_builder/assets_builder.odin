...
main :: proc() {
    ...
    // assets
    {
        file_infos := dir_path_to_file_infos("../../../assets")
        for fi in file_infos {
            if strings.has_suffix(fi.name, ".png") {
                append(&texture_paths, fmt.tprintf("../../../assets/%s", fi.name))
            } else if strings.has_suffix(fi.name, ".ldtk") {
                append(&level_paths, fmt.tprintf("../../../assets/%s", fi.name))
            }
        }
    }
    ...
    // assets/Audio/Sound
    {
        file_infos := dir_path_to_file_infos("../../../assets/Audio/Sound")
        for fi in file_infos {
            if strings.has_suffix(fi.name, ".wav") {
                append(&sound_paths, fmt.tprintf("../../../assets/Audio/Sound/%s", fi.name))
            }
        }
    }
    ...
    // assets/Audio/Music
    {
        file_infos := dir_path_to_file_infos("../../../assets/Audio/Music")
        for fi in file_infos {
            if strings.has_suffix(fi.name, ".wav") {
                append(&music_paths, fmt.tprintf("../../../assets/Audio/Music/%s", fi.name))
            }
        }
    }
    ...
    // assets/Fonts
    {
        ...
        for fi in file_infos {
            if strings.has_suffix(fi.name, ".png") {
                append(&font_paths, fmt.tprintf("../../../assets/Fonts/%s", fi.name))
