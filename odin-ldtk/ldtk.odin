...
load_from_file :: proc(filename: string, allocator := context.allocator) -> Maybe(Project) {
    // The original code for this is `data, ok := os.read_entire_file`.
    // We change it because core:os doesn't work on freestanding_wasm32 target (as expected)
    // Because there is no "operating system"
    data, ok := rl.LoadFileDataSlice(filename, allocator)
    if !ok {
        return nil
    }
